...
def pool_present(
    ...
    if IdentityPoolId is None:
        ...
        if r.get("created"):
            ...
            ret["comment"] = "A new identity pool with name {}, id {} " "is created.".format(
                IdentityPoolName, IdentityPoolId
            )
        else:
            ret["result"] = False
        ...
        if r.get("updated"):
            updated_identity_pool = r.get("identity_pool")
            ret["comment"] = "Existing identity pool with name {}, id {} " "is updated.".format(
                IdentityPoolName, IdentityPoolId
            )
        else:
            ret["result"] = False
            ret["comment"] = "Failed to update an existing identity pool {} {}: " "{}".format(
                IdentityPoolName,
                IdentityPoolId,
                r["error"].get("message", r["error"]),
            )
...
def pool_absent(
    ...
    if identity_pools is None:
        ret["result"] = True
        ret["comment"] = "No matching identity pool for the given name {}".format(IdentityPoolName)
        return ret
    ...
    if __opts__["test"]:
        ret["comment"] = "The following matched identity pools will be " "deleted.\n{}".format(
            identity_pools
        )
        ret["result"] = None
        return ret
    ...
    for identity_pool in identity_pools:
        ...
        if r.get("deleted"):
            ...
            ret["comment"] = "{}\n{}".format(ret["comment"], "{} deleted".format(change_key))
        else:
            ret["result"] = False
