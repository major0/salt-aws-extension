...
def _attributes_present(name, attributes, region, key, keyid, profile):
    ...
    if attrs_to_set:
        if __opts__["test"]:
            ...
            ret["result"] = None
            return ret
        was_set = __salt__["boto_elb.set_attributes"](name, attributes, region, key, keyid, profile)
        if was_set:
...
def _health_check_present(name, health_check, region, key, keyid, profile):
    ...
    if not health_check:
        health_check = {}
    _health_check = __salt__["boto_elb.get_health_check"](name, region, key, keyid, profile)
    ...
    if to_enable or to_disable:
        ...
        if to_enable:
            ...
            if enabled:
                ...
                ret["comment"] = "Failed to enable availability zones on {} ELB.".format(name)
                ret["result"] = False
        if to_disable:
