...
def cache_cluster_present(
    name, wait=900, security_groups=None, region=None, key=None, keyid=None, profile=None, **args
):
    """
...
def cache_cluster_absent(name, wait=600, region=None, key=None, keyid=None, profile=None, **args):
    ...
    if exists:
        ...
        deleted = __salt__["boto3_elasticache." "delete_cache_cluster"](
            name, wait=wait, region=region, key=key, keyid=keyid, profile=profile, **args
        )
        if deleted:
...
def replication_group_present(
    ...
    if check_update:
        ...
        if need_update:
            ...
            ret["comment"] = "Replication group {} is in the desired state.".format(name)
    return ret
...
def cache_subnet_group_present(
    ...
    if current:
        ...
        created = __salt__["boto3_elasticache." "create_cache_subnet_group"](
            name, subnets=subnets, region=region, key=key, keyid=keyid, profile=profile, **args
        )
        if created:
    ...
    if check_update:
        ...
        if need_update:
            ...
            ret["comment"] = "Cache subnet group {} is in the desired state.".format(name)
    return ret
