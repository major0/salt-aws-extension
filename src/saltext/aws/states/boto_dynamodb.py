...
def _add_global_secondary_index(
    ...
    if __opts__["test"]:
        ret["result"] = None
        ret["comment"] = "Dynamo table {} will have a GSI added: {}".format(name, index_name)
        return
    changes_new.setdefault("global_indexes", {})
    success = __salt__["boto_dynamodb.create_global_secondary_index"](
        name,
        __salt__["boto_dynamodb.extract_index"](gsi_config[index_name], global_index=True),
        region=region,
        key=key,
        keyid=keyid,
        profile=profile,
...
def _update_global_secondary_indexes(
    ...
    if index_updates:
        ...
        if success:
            comments.append("Updated GSIs with new throughputs {}".format(index_updates))
