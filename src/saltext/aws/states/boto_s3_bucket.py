...
def _get_canonical_id(region, key, keyid, profile):
    ret = __salt__["boto_s3_bucket.list"](region=region, key=key, keyid=keyid, profile=profile).get(
        "Owner"
    )
...
def present(
        ...
        for setter, testval, funcargs in (
            ...
            if testval is not None:
                r = __salt__["boto_s3_bucket.{}".format(setter)](
                    Bucket=Bucket, region=region, key=key, keyid=keyid, profile=profile, **funcargs
                )
    ...
    ret["comment"] = " ".join([ret["comment"], "S3 bucket {} is present.".format(Bucket)])
    ret["changes"] = {}
    _describe = __salt__["boto_s3_bucket.describe"](
        Bucket=Bucket, region=region, key=key, keyid=keyid, profile=profile
    )
        ...
        if varname == "Policy":
            if current is not None:
                ...
                if isinstance(temp, str):
                    current = __utils__["boto3.ordered"]({"Policy": salt.utils.json.loads(temp)})
    ...
    if _describe.get("Location", {}).get("LocationConstraint") != LocationConstraint:
        msg = (
            "Bucket {} location does not match desired configuration, but cannot be changed".format(
                LocationConstraint
            )
        )
        log.warning(msg)
