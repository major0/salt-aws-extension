...
def _key_present(
    ...
    if not r["result"]:
        ...
        rn = __salt__["boto_kms.create_alias"](alias, kms_key_id, region, key, keyid, profile)
        if "error" in rn:
...
def _key_rotation(key_metadata, key_rotation, region, key, keyid, profile):
    ret = {"result": True, "comment": "", "changes": {}}
    kms_key_id = key_metadata["KeyId"]
    rke = __salt__["boto_kms.get_key_rotation_status"](kms_key_id, region, key, keyid, profile)
    ...
    if not key_metadata["Enabled"]:
        ...
        return ret
    if key_rotation:
        rk = __salt__["boto_kms.enable_key_rotation"](kms_key_id, region, key, keyid, profile)
    else:
        rk = __salt__["boto_kms.enable_key_rotation"](kms_key_id, region, key, keyid, profile)
    if "error" in rk:
...
def _key_policy(key_metadata, policy, region, key, keyid, profile):
    ret = {"result": True, "comment": "", "changes": {}}
    kms_key_id = key_metadata["KeyId"]
    rkp = __salt__["boto_kms.get_key_policy"](kms_key_id, "default", region, key, keyid, profile)
