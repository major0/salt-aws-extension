...
class _Swagger:
    ...
    class SwaggerParameter:
        ...
        def location(self):
            ...
            if _location in _Swagger.SwaggerParameter.LOCATIONS:
                return _location
            raise ValueError(
                "Unsupported parameter location: {} in Parameter Object".format(_location)
            )
        ...
        def name(self):
            ...
            if _name:
                ...
                return None
            raise ValueError(
                "Parameter must have a name: {}".format(_dict_to_json_pretty(self._paramdict))
            )
        ...
        def schema(self):
            ...
            if self.location == "body":
                ...
                raise ValueError("Body parameter must have a schema: {}".format(self.name))
            return None
            ...
            for opname, opobj in ops.items():
                ...
                for rescode, resobj in opobj.get("responses").items():
                    ...
                    if model.get("type") != "object":
                        raise ValueError("model schema {} must be type object".format(modelname))
                    if "properties" not in model:
                        raise ValueError(
                            "model schema {} must have properties fields".format(modelname)
                        )
    ...
    def paths(self):
        ...
        for path in paths:
            if not path.startswith("/"):
                raise ValueError("Path object {} should start with /. Please fix it".format(path))
        return paths.items()
    ...
    def _get_current_deployment_id(self):
        ...
        stage = __salt__["boto_apigateway.describe_api_stage"](
            restApiId=self.restApiId, stageName=self._stage_name, **self._common_aws_args
        ).get("stage")
        if stage:
        ...
        if resources.get("resources"):
            ...
            for resource in res:
                delres = __salt__["boto_apigateway.delete_api_resources"](
                    restApiId=self.restApiId, path=resource.get("path"), **self._common_aws_args
                )
        ...
        if models.get("models"):
            for model in models.get("models"):
                delres = __salt__["boto_apigateway.delete_api_model"](
                    restApiId=self.restApiId, modelName=model.get("name"), **self._common_aws_args
                )
                if not delres.get("deleted"):
                    return delres
    ...
    def deploy_models(self, ret):
        ...
        for model, schema in self.models():
            ...
            if model_exists_response.get("exists"):
                update_model_schema_response = __salt__["boto_apigateway.update_api_model_schema"](
                    restApiId=self.restApiId,
                    modelName=model,
    ...
    def _deploy_method(
        ...
        if "responses" in method_data:
            ...
            raise ValueError("No responses specified for {} {}".format(resource_path, method_name))
...
def usage_plan_present(
    ...
    try:
        ...
        if not existing["plans"]:
            ...
            if __opts__["test"]:
                ret["comment"] = "a new usage plan {} would be created".format(plan_name)
                ret["result"] = None
                return ret
            ...
            if not needs_updating:
                ret["comment"] = "usage plan {} is already in a correct state".format(plan_name)
                ret["result"] = True
                return ret

            if __opts__["test"]:
                ret["comment"] = "a new usage plan {} would be updated".format(plan_name)
                ret["result"] = None
                return ret
...
def usage_plan_absent(name, plan_name, region=None, key=None, keyid=None, profile=None):
    ...
    try:
        ...
        if __opts__["test"]:
            ret["comment"] = "Usage plan {} exists and would be deleted".format(plan_name)
            ret["result"] = None
            return ret
        ...
        if "error" in result:
            ret["result"] = False
            ret["comment"] = "Failed to delete usage plan {}, {}".format(plan_name, result)
            return ret
...
def usage_plan_association_absent(
    ...
    try:
        ...
        if not plan_stages:
            ret["comment"] = "Usage plan {} has no associated stages already".format(plan_name)
            return ret
