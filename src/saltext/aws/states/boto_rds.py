...
def replica_present(
    ...
    if not replica_exists.get("exists"):
        ...
        if pmg_name != db_parameter_group_name:
            ...
            if not modified:
                ret["result"] = False
                ret["comment"] = "Failed to update parameter group of {} RDS " "instance.".format(
                    name
                )
        ...
        if len(changed) > 0:
            if __opts__["test"]:
                ret["comment"] = os.linesep.join(
                    [
                        ret["comment"],
                        "Parameters {} for group {} are set to be changed.".format(changed, name),
                    ]
                )
                ret["result"] = None
                return ret
            ...
            if "error" in update:
                ...
                ret["comment"] = os.linesep.join(
                    [
                        ret["comment"],
                        "Failed to change parameters {} for group {}:".format(changed, name),
